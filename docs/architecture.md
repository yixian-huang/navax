...
- 发布在单事务中写不可变 JSON 快照并切换当前指针；公开请求从不读取草稿表。快照由草稿经 `buildPublishedPage` 投影生成：丢弃 `enabled=false` 的分类/站点，并省略过滤后为空的分类；公开载荷不含 `enabled` 字段。预览与发布共用同一投影。
...
原生二进制更新依次执行下载、SHA-256/Ed25519 验证、SQLite 在线备份、原子替换和重启；失败保留旧版本。容器模式只检查版本，不在容器内自替换。官方域名通过 `PUBLIC_BASE_URL` 和 `INSTANCE_NAME` 配置，自部署不硬编码 `nav.ax`。子域名根域名属于可变产品设置，存放在 `system_settings.root_domain`，由管理员在后台配置，不经环境变量。

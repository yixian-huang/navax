...
export default function LayoutFull({
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 md:px-8 pt-16 md:pt-24 pb-24">
      {/* Editorial header */}
      <header className="mb-12 md:mb-16 rise-in">
        <div className="flex items-end justify-between gap-6 flex-wrap">
          <div className="min-w-0">
            {showDate && (
              <p className="text-[11px] font-medium tracking-[0.22em] uppercase text-accent-600 mb-3">
                {dateStr} · {weekDay}
              </p>
            )}
            {showGreeting && (
              <h1 className="font-heading text-4xl md:text-5xl leading-[1.05] tracking-tight text-foreground-950">
                {greeting}，
          ...
          {showClock && <div className="flex flex-col items-end flex-shrink-0">
            <span className="font-heading text-3xl md:text-4xl tabular-nums text-foreground-800 tracking-tight">
              {timeStr}
            </span>
